            ...
            <div class="container">
                <div class="row mt-5">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <?php
                            $sql = "SELECT * FROM `quizzes` WHERE `course_id` = '$var_course_token' AND `teacher_id` = '$var_teacher_token'";
                                ...
                                <table class="table table-bordered">
                                    ...
                                    while ($row = mysqli_fetch_assoc($run_sql)) { ?>
                                        <tr>
                                            ...
                                            $student = $_SESSION['ID'];
                                            $quiz_id = $row['quiz_id'];
                                            $check = "SELECT * FROM `quiz_answers` WHERE `student_id` = '$student' AND `quiz_id` = '$quiz_id'";
                                            $run_check = mysqli_query($con, $check);
                                            $num_row = mysqli_num_rows($run_check);
                                            if ($num_row == 0) { ?>
                                                <td><a href="startquiz.php?quiz=<?php echo $row['quiz_id'] ?>&teacher=<?php echo $row['teacher_id'] ?>&course=<?php echo $var_course_token ?>">Start quiz</a></td>
                                            <?php } else { ?>
                                                <td class="text-center text-success">Done</td>
                                            <?php } ?>
                                            <?php
                                            $check_marks = "SELECT `marks` FROM `quiz_grades` WHERE `student_id` = '$student' AND `teacher_id` = '$var_teacher_token' AND `quiz_id` = '$quiz_id'";
