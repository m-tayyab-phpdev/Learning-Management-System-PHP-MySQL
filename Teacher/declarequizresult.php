...
if (isset($_GET['quizid']) && isset($_GET['studentid'])) {
    $var_quiz_id = mysqli_real_escape_string($con, $_GET['quizid']);
    $var_student_id = mysqli_real_escape_string($con, $_GET['studentid']);
    $teacher_id = $_SESSION['ID'];
    $sql = "SELECT * FROM (quiz_answers a, quiz_bank b) WHERE (a.student_id = '$var_student_id' AND a.teacher_id = '$teacher_id' AND a.quiz_id = '$var_quiz_id' AND a.bank_id = b.bank_id)";
    $run_sql = mysqli_query($con, $sql);
}

if (isset($_POST['quiz-marks'])) {
    $var_quiz = mysqli_real_escape_string($con, $_POST['quizid']);
    ...
    $marks = "INSERT INTO `quiz_grades`(`student_id`, `quiz_id`, `teacher_id`, `marks`) VALUES ('$var_student','$var_quiz','$var_teacher','$var_marks')";
    $run_marks = mysqli_query($con, $marks);
    if ($run_marks) {
        echo "<script>window.location.replace('quizresult.php')</script>";
    }
}
?>
        ...
        <div class="container-fluid px-4">
            ...
            <div class="container">
                ...
                <div class="card">
                    ...
                    <?php
                    $check = "SELECT * FROM `quiz_grades` WHERE `student_id` = '$var_student_id' AND `teacher_id` = '$teacher_id' AND `quiz_id` = '$var_quiz_id'";
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-8">
                                <form action="" method="post">
                                    ...
                                    <input type="hidden" name="quizid" value="<?php echo $var_quiz_id ?>">
                                    <input type="hidden" name="teacherid" value="<?php echo $teacher_id ?>">

                            </div>
                            <div class="col-md-2">
                                <button type="submit" name="quiz-marks"><i class="fas fa-upload"></i></button>
                                </form>
                            </div>
                        </div>
                    <?php }else{ ?>
